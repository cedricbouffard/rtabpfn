...
if (!has_unsupervised) {
  message("Note: tabpfn-extensions[unsupervised] not installed. Some examples will be skipped.")
}

# Detection of CI environment
is_ci <- Sys.getenv("CI") != "" || Sys.getenv("GITHUB_ACTIONS") == "true"

# Set eval to FALSE if extension not available or on CI
knitr::opts_chunk$set(eval = has_unsupervised && !is_ci)
```
...
```{r with-prediction, message=FALSE, warning=FALSE, fig.width=6, fig.height=5}
