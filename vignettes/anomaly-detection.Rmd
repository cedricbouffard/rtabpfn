...
has_unsupervised <- tryCatch({
  ...
  message("Note: Error checking unsupervised availability: ", e$message)
  FALSE
})

if (!has_unsupervised) {
  message("Note: tabpfn-extensions[unsupervised] not installed. Some examples will be skipped.")
  message("Install with: reticulate::py_install('tabpfn-extensions[unsupervised]', pip = TRUE)")
}
# Don't skip all evaluation - let at least some code run
# But set eval=FALSE if extension not available
if (!has_unsupervised) {
