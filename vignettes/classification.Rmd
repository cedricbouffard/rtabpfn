---
title: "Classification with TabPFN"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Classification with TabPFN}
...
The `rtabpfn` package provides a powerful interface for classification tasks using the TabPFN (Tabular Prior-Fitted Network) foundation model. TabPFN excels at tabular classification tasks, particularly on small datasets, by leveraging prior knowledge from millions of datasets.
...
```{r classification-basic, message=FALSE, warning=FALSE}
# Load data
data(iris)
...
TabPFN can provide class probabilities, offering insight into prediction confidence:

```{r classification-probs, message=FALSE, warning=FALSE}
...
```{r plot-confusion, message=FALSE, warning=FALSE, fig.width=5, fig.height=5}
...
```{r plot-probabilities, message=FALSE, warning=FALSE, fig.width=8, fig.height=4}
...
```{r plot-feature-space, message=FALSE, warning=FALSE, fig.width=6, fig.height=5}
...
```{r plot-confidence, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
```{r multiclass, message=FALSE, warning=FALSE}
...
```{r plot-prob-comparison, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
```{r binary-classification, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
TabPFN provides excellent classification performance with intuitive probability estimates. Key features:

-   **Multi-class support**: Handles binary and multi-class problems natively
...
The model is particularly effective for: - Quick prototyping on tabular data - Small to medium-sized datasets - Problems requiring probability estimates - Rapid iteration without extensive tuning
