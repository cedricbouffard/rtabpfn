...
has_shap <- check_shap_available()
if (!has_shap) {
  message("Note: tabpfn-extensions[all] with SHAP not installed. Some examples will be skipped.")
}

# Detection of CI environment
is_ci <- Sys.getenv("CI") != "" || Sys.getenv("GITHUB_ACTIONS") == "true"

knitr::opts_chunk$set(eval = has_shap && !is_ci)
```
...
```{r shap-values, message=FALSE, warning=FALSE}
...
```{r plot-shap-summary, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
# Plot SHAP summary
...
```{r manual-importance, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
```{r individual-prediction, message=FALSE, warning=FALSE}
...
```{r dependence-plot, message=FALSE, warning=FALSE, fig.width=6, fig.height=5}
# SHAP dependence for 'wt' (weight)
p2 <- plot_shap_dependence(shap_vals, X, feature = "wt")
print(p2)
```

```{r dependence-plot-2, message=FALSE, warning=FALSE, fig.width=6, fig.height=5}
...
```{r manual-dependence, message=FALSE, warning=FALSE, fig.width=6, fig.height=5}
...
```{r beeswarm-plot, message=FALSE, warning=FALSE, fig.width=6, fig.height=8}
...
```{r feature-value-shap, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
...
```{r interaction-plot, message=FALSE, warning=FALSE, fig.width=6, fig.height=5}
...
```{r decomposition-plot, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
```{r classification-shap, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
